...
4. **On failure:** Alert the user and halt. No further rows are uploaded. The error is passed through `describeUploadError()` so the user sees the same friendly wording as the connection test (a fetch `TypeError` after a passed pre-flight is reported as a lost connection). The raw SDK error is not shown on screen; a **Copy technical details** button puts a support report on the clipboard instead.
...
Probes use a dedicated `S3Client` with `maxAttempts: 1` and a 15-second `AbortController` timeout so a failing test answers quickly. Results are `{ ok, code, severity, title, detail, raw, warnings }`; `raw` carries the underlying SDK/browser error text. It is deliberately not displayed — non-technical users find it confusing — but every failure alert has a **Copy technical details** button that puts a report from `formatDebugReport()` on the clipboard: timestamp, page URL, user agent, bucket, region, the last four characters of the key ID (never the secret), outcome code, messages and the raw error, plus the failed row and batch ID for mid-upload failures. Cases that cannot be told apart from the browser — wrong bucket name vs. wrong region vs. missing CORS, and deleted vs. deactivated keys — are grouped under a single message whose remedy is the same for each.
